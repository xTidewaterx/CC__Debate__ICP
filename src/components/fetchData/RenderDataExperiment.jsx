...
//add something to change loading if there is no data to fetch
//if we get the promise returned with the intel that no data is avialbel
//then fetch is returned with promise and loading is done, just there is no data to get from database
//recieve data as datasnapshot, read data from the snapshot, efficiently generated
//we check current data and initial value in array, if match, then all data is scanned, we are at beginning again
...
export const RenderAllLevels = () => {
  ...
  if (state) {
    ...
    const renderMatches = (currentId, currentCl) => {
      return sortedArray.map((data) => {
        ...
        if (data.parentId == currentId && currentCl < 3) {
          ...
          const { organismPublicName, cl, ID, parentId, comment, likeCount } = data;
          return (
            <div className="organismBattlefield__Container">
             <Avatar object={{cl, organismPublicName, ID, comment}} />
  
              <li
                className="organismBattlefield__ContainerItem"
                style={{ marginLeft: currentLevelStyling + "px" }}
              >
                id: {ID} parentId: {parentId} cl: {cl} comment: {comment}
                <div className="organismBattlefield__likeReplyContainer">
                  <Like
                    className="organismBattlefield__Like"
                    likeCount={likeCount}
                    idValue={ID}
    ...
    return (
      <div className="organismBattlefield">
        <Comment organismObject={{ state, setState }} />
        {
          //for each we add to one object if param is single function

          sortedArray.map((data) => {
            const { organismPublicName, cl, ID, comment } = data;
            if (data.cl == 0) {
              return (
                <div className="organismBattlefield__Container">
                 <div className="organismBattlefield__ContainerDetour"> <Avatar object={{cl, organismPublicName, ID, comment}} />

                  </div><li className="organismBattlefield__ContainerItem">
                    {ID} cl: {cl}  organismComment: {comment}
                    <div className="organismBattlefield__likeReplyContainer">
                      <Like
